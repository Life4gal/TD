...
namespace systems
{
	auto HealthBar::render(entt::registry& registry, sf::RenderWindow& window) noexcept -> void
	{
		using namespace components;

		auto& [triangles] = registry.ctx().get<health_bar::Renderer>();
		...
		// 2个矩形 * 6个顶点(每个矩形2个三角形)
		triangles.resize(enemy_count * 12);
