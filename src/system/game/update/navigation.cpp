...
namespace game::system::update
{
	auto navigation(entt::registry& registry, const sf::Time delta) noexcept -> void
	{
		...
		// 地面导航
		for (const auto enemy_view = registry.view<tags::enemy::identifier, tags::enemy::status::alive, tags::enemy::archetype::ground, entity::Position, const enemy::Movement>();
		     auto [entity, position, movement]: enemy_view.each())
		{
			...
			while (remaining_move_distance > 0)
			{
				...
				const auto current_point_center_position = tile_map.coordinate_grid_to_world(current_point);
